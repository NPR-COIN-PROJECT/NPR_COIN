...
#if defined(NDEBUG)
# error "NEPLEASECOIN cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5fab956bd0c3e01b6e3f3378f64f01af0e05c2fb0b5389e865a3fd4971c0fe1f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NEPLEASECOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NEPLEASECOIN Signed Message:\n";
...
{
    // NEPLEASECOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NEPLEASECOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NEPLEASECOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1526892349, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "LETS BREAK THE INDIAN RULE ON OUR CURRENCY";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045c612d26d615ac3b2f68287acb557e2c77d2d8725b9e59d6962bb04d2184efbac95dd945fed6483b23fa62959042d0c4fc58b0ecdab6760f2e291ff2781500b9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526892349;
        ...
        assert(block.hashMerkleRoot == uint256("0x1114ba43b7d187a8c1ec535e7ece2863f6758af4fe102a92ab55ebdbbb02b0fd"));
...
unsigned char pchMessageStart[4] = { 0x6c, 0xa0, 0xa5, 0x22 }; // NEPLEASECOIN: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NEPLEASECOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NEPLEASECOINMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NEPLEASECOINMiner(CWallet *pwallet)
{
    printf("NEPLEASECOINMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nepleasecoin-miner");
    ...
    try { loop {
        ...
        printf("Running NEPLEASECOINMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NEPLEASECOINMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NEPLEASECOINMiner, pwallet));
}
